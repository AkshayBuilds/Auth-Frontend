...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between w-full items-center p-4 sm:p-6 sm:px-24 absolute top-0'> <img src={assets.logo} alt="" className='w-28 sm:w-32'/> {userData ? <div
...
</div> : <button onClick={() => navigate('/login')} className='outline-none flex items-center gap-2 text-gray-800 border-2 border-gray-500 rounded-full px-6 py-2 hover:bg-gray-300 transition-all duration-300 cursor-pointer'>Login <img src={assets.arrow_icon} alt="" /></button> } </div>
  )
}
